...
public class NBTConverter {
    ...
    public enum UuidValueType {
        ...
        public void writeIdString(@Nullable UUID uuid, NBTTagCompound tag) {
            tag.setString(idFieldName, uuid == null ? "" : encodeUuid(uuid));
        }
        ...
        public Optional<UUID> tryReadIdString(NBTTagCompound tag) {
            ...
            return Optional.of(decodeUuid(questId));
        }
        ...
        for (Entry<String, JsonElement> entry : jObj.entrySet()) {
            ...
            if (!format) {
                ...
                try {
                    ...
                    if (tags.hasKey(key)) {
                        VendingMachine.LOG.warn("JSON/NBT formatting conflict on key '{}'. Skipping...", key);
                        continue;
                    }
                }
    ...
    private static NBTBase JSONtoNBT_Element(JsonElement jObj, byte id, boolean format) {
        ...
        try {
            if (
                tagID == 1 && (id <= 0 || jObj.getAsJsonPrimitive()
                    .isBoolean())
            ) // Edge case for BQ2 legacy files
            {
                ...
                if (jObj.isJsonArray()) {
                    ...
                    for (Entry<String, JsonElement> entry : jAry.entrySet()) {
                        try {
                            ...
                            VendingMachine.LOG.error("Missing json key type for key: {}", entry.getKey());
            ...
            VendingMachine.LOG.error("An error occured while parsing JsonElement to NBTBase ({}):", tagID, e);
        }

        VendingMachine.LOG.error("Unknown NBT representation for {} (ID: {})", jObj.toString(), tagID);
        return new NBTTagString();
    }
        ...
        if (jObj.isJsonPrimitive()) {
            ...
            if (prim.isNumber()) {
                if (
                    prim.getAsString()
                        .contains(".")
                ) // Just in case we'll choose the largest possible container supporting this number
                  // type (Long or Double)
                {
                    tagID = 6;
                } else {
    ...
    static {
        try {
            ...
            try {
                ...
                VendingMachine.LOG.error("Unable to hook into NBTTagList!", e2);
            }
        }
    }
}
