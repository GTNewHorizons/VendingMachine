...
public final class PacketAssembly {
    ...
    public List<NBTTagCompound> splitPacket(NBTTagCompound tags) {
        try {
            ...
            VendingMachine.LOG.error("Unable to split build packet!", e);
    ...
    public NBTTagCompound assemblePacket(UUID owner, NBTTagCompound tags) {
        ...
        if (tmp == null) {
            ...
            VendingMachine.LOG.error("Unexpected change in packet byte length: " + size + " > " + tmp.length);
            clearBuffer(owner);
            return null;
        }
        ...
        if (end) {
            ...
            try {
                ...
                throw new RuntimeException("Unable to assemble packet", e);
            }
        }
            ...
            synchronized (buffer) {
                if (buffer.containsKey(owner)) {
                    throw new IllegalStateException(
                        "Attepted to start more than one packet assembly for UUID " + owner.toString());
                }
